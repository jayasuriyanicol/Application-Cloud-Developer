...
class Booking(ABC):
    def __init__(self, booking_id: str, patient_name: str, doctor: str, 
                 department: str, date: str, time: str, status: str = "scheduled"):
    ...
    def estimated_wait(self, factor: float = 1.0) -> int:
        ...
        priority = self.priority_level()
        return int((duration * factor) + (priority * 5))


class MedicalVisit(Booking):

    def __init__(self, booking_id: str, patient_name: str, doctor: str, 
                 department: str, date: str, time: str, 
                 visit_reason: str, first_time: bool, status: str = "scheduled"):
    ...
    def priority_level(self) -> int:
        ...
        reason = (self.visit_reason or "").lower()
        keywords = ["urgente", "dolore", "acuto", "svenimento", "chest pain"]
...
class DiagnosticExam(Booking):
    def __init__(self, booking_id: str, patient_name: str, doctor: str, 
                 department: str, date: str, time: str, 
                 exam_type: str, requires_fasting: bool, status: str = "scheduled"):
    ...
    def base_duration(self) -> int:
        ...
        if "rmn" in et or "mri" in et:
            return 60
        
        if "tac" in et or "ct" in et:
            return 45
    ...
    def priority_level(self) -> int:
        ...
        if et in ["rmn", "mri", "tac", "ct", "biopsia"]:
            return 9
        return 7
...
if __name__ == "__main__":
    ...
    print("--- INIZIALIZZAZIONE SISTEMA CLINICA ---\n")
    
    my_clinic = ClinicHub()

    visit1 = MedicalVisit(
        booking_id="BK-VIS-001",
        patient_name="Mario Rossi",
        doctor="Dr. House",
        department="Medicina Interna",
        date="2026-02-15",
        time="10:00",
        visit_reason="Dolore acuto al fianco destro",
        first_time=True
    )

    exam1 = DiagnosticExam(
        booking_id="BK-EXM-099",
        patient_name="Luigi Verdi",
        doctor="Dr. Strange",
        department="Radiologia",
        date="2026-02-16",
        time="14:30",
        exam_type="RMN Encefalo",
        requires_fasting=False
    )

    print(f"Adding {visit1.booking_id}: {my_clinic.add(visit1)}")
    print(f"Adding {exam1.booking_id}: {my_clinic.add(exam1)}")

    print(f"\nAttesa stimata {visit1.booking_id}: {visit1.estimated_wait()} min")
    
    print(f"Attesa stimata {exam1.booking_id} (fattore 1.2): {exam1.estimated_wait(factor=1.2)} min")

    print(f"\nCambio stato {visit1.booking_id} a 'checked_in'...")
    my_clinic.patch_status("BK-VIS-001", "checked_in")
    
    updated_exam = DiagnosticExam(
        booking_id="BK-EXM-099",
        patient_name="Luigi Verdi",
        doctor="Dr. Strange",
        department="Radiologia",
    ...
    print(f"Aggiornamento completo (PUT) di {exam1.booking_id}...")
    my_clinic.update("BK-EXM-099", updated_exam)

    print("\n--- LISTA PRENOTAZIONI ATTUALI ---")
    all_bookings = my_clinic.list_all()
