...
Signature will always contain 3 numbers; n will always be a non-negative number; if n == 0, then return an empty array (except in C return NULL) and be ready for 
...
def tribonacci(signature:list[int] ,n:int):
    ...
    if len(signature) < 3:
        ...
        print("ATTENZIONE ! la lunghezza della SEGNATURA non può essere inferiore ai 3 elementi !")
    ...
    if n < 3:
        ...
        sequenzaTribonacci = signature[:n]
    else:
        sequenzaTribonacci = signature[:]

    while len(sequenzaTribonacci) < n:


        prossimoElementoSequenzaTribonacci:int = sequenzaTribonacci[-1] + sequenzaTribonacci[-2] + sequenzaTribonacci[-3]
        sequenzaTribonacci.append(prossimoElementoSequenzaTribonacci)

    return sequenzaTribonacci




'''Eseguiamo da soli dei DRIVER PROGRAMM al fine di testare la veridicità delle funzioni'''
