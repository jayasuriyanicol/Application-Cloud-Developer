...
public class ImpiegatoServiceImpl implements ImpiegatoService  {
	...
	//Adding the more complicated method, requested from the prompt exercise
	...
	public ImpiegatoNomeCognomeDTO eliminaMatricolaNomCogn(int matricola) {
		...
		if(opt.isPresent()) {
			...
			ImpiegatoDTO eliminaImpiegato = cancellaID(matricola);
			
			return new ImpiegatoNomeCognomeDTO(eliminaImpiegato.getNome(), eliminaImpiegato.getCognome());
	...
	public List<ImpiegatoNomeCognomeDTO> listaNomCogn() {
		...
		return dao.ImpiegatoOrdMatricola()
				
				.stream()
		
				.map(imp -> new ImpiegatoNomeCognomeDTO(imp.getNome(), imp.getCognome()))
	...
	public Double totaleSalari() {
		...
		Double totale = dao.totaleSalario();
	...
	public List<ImpiegatoDTO> listaOrdCognome() {
		...
		return new ArrayList<ImpiegatoDTO>(MapperImpiegato.CollectionImpiegatoEntityToDTO(dao.ImpiegatoOrdCognome()));
	...
	public ImpiegatoDTO assuntoMaxTemp() {
		...
		return MapperImpiegato.ImpiegatoEntityToDTO(dao.findAll()
				.stream()
	...
	public ImpiegatoDTO salarioMaxDatSPec(LocalDateTime dataSpecifica) {
	    ...
	    //Created a list of all the MaxSalary, and keeping the first one by the date and convert into DTO
	    List<Impiegato> lista = dao.salarioMaxDatSPec(dataSpecifica);
	    
	    
	    if (!lista.isEmpty()) {
	   
	        Impiegato imp = lista.get(0); 
	        
	       
	        return MapperImpiegato.ImpiegatoEntityToDTO(imp);
	    }
